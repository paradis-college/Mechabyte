...
const sketch = (p: p5) => {
  ...
  p.draw = () => {
    ...
    p.textSize(11);
    p.fill(200);
    p.text('Keeps intake angle constant while raising/lowering', p.width / 2, 50);
  ...
  const drawPantograph = (p: p5, angle: number) => {
    ...
    const p2 = { x: baseX + segmentLength * Math.cos(extension), y: baseY - segmentLength * Math.sin(extension) };
    const p3 = { x: p2.x + segmentLength * Math.cos(extension), y: p2.y - segmentLength * Math.sin(extension) };
    const p4 = { x: baseX + segmentLength * Math.cos(extension), y: baseY - segmentLength * Math.sin(extension) };
    ...
    p.textSize(8);
    p.fill(200, 200, 100);
    p.text('Evolution: 1 servo → 2 servos → Tetrix 40:1 motor', p.width / 2, p.height - 20);
    
    // Team attribution
    p.textSize(8);
    p.fill(150);
    p.text('Designed by: David Grigore, Ștefan Albu', p.width / 2 + 150, p.height - 20);
  };
};
...
<template>
  <div class="animation-wrapper">
    ...
    <p class="animation-caption">Pantograph mechanism keeps claw at constant angle during vertical movement</p>
  </div>
</template>
