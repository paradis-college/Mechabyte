...
const sketch = (p: p5) => {
  ...
  const drawLauncher = (
    ...
    if (!isReleased) {
      ...
      p.fill(200);
      p.text(
        `Tension: ${Math.floor((tension / 60) * 100)}%`,
        p.width / 2,
        p.height - 45,
      );
    } else {
    ...
    p.fill(150);
    p.text(
      "Built by: Cristian Ghidireac, David Grigore",
      p.width / 2,
      p.height - 20,
    );
  };
...
<template>
  <div class="animation-wrapper">
    ...
    <p class="animation-caption">
      Drone launcher: Elastic band held under tension by servo-controlled rod,
      released during endgame
    </p>
  </div>
</template>
