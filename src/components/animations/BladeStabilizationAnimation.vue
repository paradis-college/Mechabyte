...
const sketch = (p: p5) => {
  ...
  const drawSolution = (p: p5) => {
    ...
    p.stroke(200);
    p.strokeWeight(2);
    for (let angle of [p.QUARTER_PI, -p.QUARTER_PI, p.PI - p.QUARTER_PI, p.PI + p.QUARTER_PI]) {
    ...
    p.textSize(8);
    p.fill(150);
    p.text('Solution designed and 3D-printed by: Ianis Cotoc, Teodor Matricală', centerX, p.height - 20);
  };
};
...
<template>
  <div class="animation-wrapper">
    ...
    <p class="animation-caption">Problem: Rotating blades align parallel and fail. Solution: 3D-printed hexagonal discs lock blades at 90°</p>
  </div>
</template>
