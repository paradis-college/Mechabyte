...
onMounted(() => {
  ...
  const sketch = (p: p5) => {
    ...
    function drawZones(p: p5) {
      ...
      p.fill(0, 100, 255, 30);
      p.noStroke();
      p.rect(FIELD_SIZE - TILE_SIZE, FIELD_SIZE - TILE_SIZE, TILE_SIZE, TILE_SIZE);
      ...
      p.textSize(12);
      p.text('Net Zone', FIELD_SIZE - TILE_SIZE / 2, FIELD_SIZE - TILE_SIZE / 2);
      p.textSize(10);
      p.text('2 pts', FIELD_SIZE - TILE_SIZE / 2, FIELD_SIZE - TILE_SIZE / 2 + 15);
    }
    ...
    function drawSubmersible(p: p5) {
      ...
      const sampleColors = [[255, 0, 0], [255, 255, 0], [0, 100, 255]];
      for (let i = 0; i < 3; i++) {
    ...
    function drawRobot(p: p5) {
      ...
      p.fill(0, 255, 0);
      p.noStroke();
      p.rect(-ROBOT_SIZE / 4, -ROBOT_SIZE / 4, ROBOT_SIZE / 2, ROBOT_SIZE / 2, 2);
      
      // Direction indicator
      p.fill(255, 215, 0);
      p.triangle(
        -ROBOT_SIZE * 0.2, 0,
        ROBOT_SIZE * 0.3, -ROBOT_SIZE * 0.15,
        ROBOT_SIZE * 0.3, ROBOT_SIZE * 0.15
      );
    ...
    function drawLabels(p: p5) {
      ...
      // Start position
      p.textSize(11);
      p.text('Start', START_POS.x * TILE_SIZE, FIELD_SIZE - START_POS.y * TILE_SIZE + 40);
    ...
    function updatePhase(p: p5) {
      ...
      switch (phase) {
        ...
        case PHASE_MOVE_TO_SUBMERSIBLE:
          ...
          robotX = p.lerp(START_POS.x * TILE_SIZE, SUBMERSIBLE_POS.x * TILE_SIZE, easeT);
          robotY = p.lerp(FIELD_SIZE - START_POS.y * TILE_SIZE, FIELD_SIZE - SUBMERSIBLE_POS.y * TILE_SIZE, easeT);
        ...
        case PHASE_MOVE_TO_NET:
          ...
          robotX = p.lerp(SUBMERSIBLE_POS.x * TILE_SIZE, NET_ZONE_POS.x * TILE_SIZE, easeT);
          robotY = p.lerp(FIELD_SIZE - SUBMERSIBLE_POS.y * TILE_SIZE, FIELD_SIZE - NET_ZONE_POS.y * TILE_SIZE, easeT);
...
<template>
  <div class="animation-container">
    ...
    <p class="animation-caption">
      Into the Deep autonomous: Robot starts from observation zone, moves to submersible to collect sample (~1.5s), 
      then delivers to net zone for scoring (~2.0s). Total autonomous time: ~3.5s, earning 2 points.
    </p>
  </div>
</template>
