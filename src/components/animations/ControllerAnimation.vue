...
const controllerMappings = {
  ...
  "into-the-deep": {
    ...
    2: [
      {
        button: "D-Pad Up/Down",
        action: "Extend/Retract Slider",
        color: COLORS.PRIMARY,
      },
      {
        button: "Left Stick Y",
        action: "Raise/Lower Intake",
        color: COLORS.PRIMARY,
      },
      { button: "Left Bumper", action: "Open Intake", color: COLORS.WARNING },
      {
        button: "Right Bumper",
        action: "Close Intake & Out-take",
        color: COLORS.DANGER,
      },
    ],
  },
...
const sketch = (p: p5) => {
  ...
  const drawButtonIndicators = (
    p: p5,
    mappings: (typeof controllerMappings)["centerstage"][1],
  ) => {
    const scale = Math.min(p.width, p.height) / 400;
...
watch(
  () => [props.season, props.driverNumber],
  () => {
    if (p5Instance) {
      p5Instance.remove();
      p5Instance = null;
