...
const sketch = (p: p5) => {
  ...
  const drawField = (p: p5) => {
    ...
    for (let i = 0; i < 6; i++) {
      // Column labels (A-F)
      p.text(String.fromCharCode(65 + i), fieldX + i * tileSize + tileSize / 2, fieldY - 12);
      // Row labels (1-6)
      p.text((i + 1).toString(), fieldX - 12, fieldY + i * tileSize + tileSize / 2);
    }
    ...
    p.stroke(180);
    p.strokeWeight(5);
    p.line(fieldX + 20, fieldY + fieldSize / 2, fieldX + fieldSize - 20, fieldY + fieldSize / 2);
    p.textSize(11);
    p.fill(180);
    p.noStroke();
  ...
  const drawBackstageTrajectory = (p: p5, progress: number) => {
    ...
    p.textSize(12);
    p.fill(200);
    p.text('Robot faces backstage - drives straight forward to parking zone', p.width / 2, 568);
  ...
  const drawAudienceTrajectory = (p: p5, progress: number) => {
    ...
    // First segment (B6 → C2)
    const dist1 = Math.sqrt(Math.pow((waypointX - startX) / tileSize, 2) + Math.pow((waypointY - startY) / tileSize, 2)) * 2;
    p.text(`Seg 1: ${dist1.toFixed(1)}ft`, (startX + waypointX) / 2 + 10, (startY + waypointY) / 2);
    p.text('~3.0s', (startX + waypointX) / 2 + 10, (startY + waypointY) / 2 + 18);
    
    // Second segment (C2 → A1)
    const dist2 = Math.sqrt(Math.pow((endX - waypointX) / tileSize, 2) + Math.pow((endY - waypointY) / tileSize, 2)) * 2;
    p.text(`Seg 2: ${dist2.toFixed(1)}ft`, (waypointX + endX) / 2 + 10, (waypointY + endY) / 2);
    ...
    if (progress < 0.6) {
      ...
      robotAngle = Math.atan2(waypointY - startY, waypointX - startX) - p.HALF_PI;
    } else {
    ...
    p.textSize(12);
    p.fill(200);
    p.text('Robot faces audience - drives forward to C2, then strafes left to backstage', p.width / 2, 568);
    p.text('Total time: ~4.5s (uses waypoint for navigation)', p.width / 2, 590);
  };

  const drawRobot = (p: p5, x: number, y: number, angle: number, progress: number) => {
    p.push();
    p.translate(x, y);
    p.rotate(angle);
    ...
    // Direction arrow
    p.fill(255, 200, 0);
    p.triangle(0, -robotSize / 2 - 3, -6, -robotSize / 2 - 12, 6, -robotSize / 2 - 12);
    ...
    if (progress > 0.05 && progress < 0.95) {
      ...
      p.strokeWeight(2);
      for (let i = 0; i < 3; i++) {
        p.line(-robotSize / 2 - 5 - i * 4, -robotSize / 4 + i * 6, -robotSize / 2 - 10 - i * 4, -robotSize / 4 + i * 6);
      }
    }
  ...
  const drawDirectionArrow = (p: p5, x1: number, y1: number, x2: number, y2: number) => {
...
<template>
  <div class="animation-wrapper">
    ...
    <p class="animation-caption">
      Autonomous trajectories with timing and velocity annotations: backstage-facing (1.5s) vs audience-facing (4.5s via C2 waypoint)
    </p>
  </div>
</template>
