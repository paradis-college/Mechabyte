...
const sketch = (p: p5) => {
  ...
  const drawVersionIndicator = (p: p5, version: number) => {
    ...
    p.textSize(12);
    p.fill(version === 0 ? 255 : 150, version === 0 ? 180 : 150, version === 0 ? 0 : 100);
    p.text('V1: Brush', startX, y + 25);
    
    // V2 - Precision Claw
    p.fill(version === 1 ? 255 : 100, version === 1 ? 200 : 100, 0, version === 1 ? 255 : 100);
    p.noStroke();
    p.circle(p.width / 2, y, 16);
    p.textSize(12);
    p.fill(version === 1 ? 255 : 150, version === 1 ? 200 : 150, version === 1 ? 0 : 100);
  ...
  const drawClawV2 = (p: p5) => {
    ...
    p.textSize(10);
    p.fill(120, 120, 140);
    p.text('Iteration 2: Rareș Cozma, Cristian Ghidireac', centerX, p.height - 25);
  };
  ...
  const drawClawV3 = (p: p5) => {
    ...
    p.textSize(10);
    p.fill(120, 140, 140);
    p.text('Final Design: David Grigore, Cristian Ghidireac, Rareș Cozma', centerX, p.height - 25);
  };

  const drawToothedWheel = (p: p5, x: number, y: number, radius: number, teeth: number, enhanced: boolean) => {
    p.push();
    p.translate(x, y);
    ...
    for (let i = 0; i < teeth; i++) {
      ...
      p.strokeWeight(2);
      p.beginShape();
      p.vertex(baseRadius * p.cos(angle - toothWidth), baseRadius * p.sin(angle - toothWidth));
