...
onMounted(() => {
  ...
  const sketch = (p: p5) => {
    ...
    p.setup = () => {
      const canvas = p.createCanvas(FIELD_PIXEL_SIZE + 100, FIELD_PIXEL_SIZE + 200)
    ...
    function drawField() {
      ...
      for (let i = 1; i < 4; i++) {
        ...
        p.line(submersibleX + 10, rungY, submersibleX + SUBMERSIBLE_SIZE - 10, rungY)
      }
      ...
      p.text('4ft × 4ft', FIELD_PIXEL_SIZE / 2, submersibleY + SUBMERSIBLE_SIZE + 15)
      ...
      p.text('2ft × 2ft', basketX + BASKET_SIZE / 2, basketY + BASKET_SIZE + 30)
      
      // Net Zone (bottom left)
      p.stroke(0, 255, 100)
      p.strokeWeight(2)
      p.fill(0, 255, 100, 30)
      p.rect(10, FIELD_PIXEL_SIZE - NET_ZONE_SIZE - 10, NET_ZONE_SIZE, NET_ZONE_SIZE)
      
      p.noStroke()
      p.fill(0, 255, 100)
      p.textSize(12)
      p.text('Net Zone', 10 + NET_ZONE_SIZE / 2, FIELD_PIXEL_SIZE - 10)
      p.text('2 pts', 10 + NET_ZONE_SIZE / 2, FIELD_PIXEL_SIZE - NET_ZONE_SIZE - 15)
    ...
    function updateRobot() {
      ...
      const easeProgress = (t: number) => t < 0.5 ? 2 * t * t : -1 + (4 - 2 * t) * t
      ...
      switch (currentPhase) {
        ...
        case PHASES.NAVIGATE_TO_BASKET:
          ...
          if (elapsed < 4000) {
            ...
            robotY = p.lerp(FIELD_PIXEL_SIZE - SUBMERSIBLE_SIZE / 2, targetY, t)
        ...
        case PHASES.RETURN:
          ...
          if (elapsed < 4000) {
            const t = easeProgress(elapsed / 4000)
            robotX = p.lerp(FIELD_PIXEL_SIZE - BASKET_SIZE - 10 + BASKET_SIZE / 2, targetX, t)
    ...
    function drawRobot() {
      ...
      if (sliderHeight > 0) {
        ...
        p.textAlign(p.CENTER, p.CENTER)
        p.textSize(10)
        p.text(`${Math.round(sliderHeight / FIELD_SCALE)}"`, 15, -ROBOT_SIZE / 2 - sliderHeight / 2)
      }
      ...
      p.fill(255)
      p.noStroke()
      p.triangle(0, -ROBOT_SIZE / 3, -5, -ROBOT_SIZE / 3 + 8, 5, -ROBOT_SIZE / 3 + 8)
      ...
      if (currentPhase === PHASES.NAVIGATE_TO_BASKET || currentPhase === PHASES.RETURN) {
    ...
    function drawStatus() {
      ...
      p.fill(150, 150, 255)
      p.textSize(12)
      p.text('High Basket: 8 pts/sample  •  Net Zone: 2 pts/sample', 20, boxY + 120)
    }
  }
