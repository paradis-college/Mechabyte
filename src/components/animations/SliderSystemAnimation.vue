...
const sketch = (p: p5) => {
  ...
  p.draw = () => {
    ...
    if (currentPhase === PHASE_RETRACTED && phaseTimer > phaseDuration) {
      ...
      targetExtension = 0;
      phaseTimer = 0;
    } else if (currentPhase === PHASE_RETRACTING && phaseTimer > phaseDuration) {
  ...
  const drawSliderSystem = (p: p5, extension: number) => {
    ...
    p.strokeWeight(4);
    p.line(baseX - 30, baseY - 30, baseX - 30, baseY - 30 - maxSliderHeight - 20);
    p.line(baseX + 30, baseY - 30, baseX + 30, baseY - 30 - maxSliderHeight - 20);
    ...
    p.line(gripperX, armY - gripperOpen, gripperX + 15, armY - gripperOpen - 10);
    p.line(gripperX, armY + gripperOpen, gripperX + 15, armY + gripperOpen + 10);
    ...
    p.fill(0, 255, 100);
    p.textSize(11);
    p.text(`${Math.round(extension * 100)}%`, baseX - 70, (baseY - 30 + carriageY) / 2);
    p.textSize(9);
    p.text('Extended', baseX - 70, (baseY - 30 + carriageY) / 2 + 15);
  };

  const drawArrow = (p: p5, x1: number, y1: number, x2: number, y2: number, size: number) => {
    p.stroke(0, 255, 100);
    p.strokeWeight(1);
    p.line(x1, y1, x2, y2);
