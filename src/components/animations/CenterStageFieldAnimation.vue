...
const sketch = (p: p5) => {
  ...
  let pixels: Array<{ x: number; y: number; collected: boolean; placedOnBackdrop: boolean }> = [];
  ...
  const drawField = (p: p5) => {
    ...
    p.rect(FIELD_SIZE - backdropWidth, FIELD_SIZE / 2 - backdropHeight / 2, backdropWidth, backdropHeight);
    p.noStroke();
    p.fill(0, 200, 255);
    p.textSize(11);
    p.text('Backdrop', FIELD_SIZE - backdropWidth / 2, FIELD_SIZE / 2 - backdropHeight / 2 + 15);
    p.text('2ft × 4ft', FIELD_SIZE - backdropWidth / 2, FIELD_SIZE / 2 - backdropHeight / 2 + 30);
    ...
    p.rect(FIELD_SIZE - backdropWidth, backstageY, backdropWidth, TILE_SIZE * 1.5);
    p.noStroke();
    p.fill(0, 200, 255);
    p.textSize(10);
  ...
  const drawHexagon = (p: p5, x: number, y: number, size: number, color: p5.Color) => {
    p.push();
    p.translate(x, y);
    p.fill(color);
...
<template>
  <div class="animation-wrapper">
    ...
    <p class="animation-caption">CenterStage Game Field (12ft × 12ft): Robot navigates to collect hexagonal pixels from spike marks, places them on the backdrop, and launches a paper drone over the truss into the backstage landing zone.</p>
  </div>
</template>
