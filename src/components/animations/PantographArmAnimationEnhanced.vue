...
const sketch = (p: p5) => {
  ...
  const drawTetrixMotor = (p: p5, angle: number) => {
    ...
    for (let i = 0; i < 8; i++) {
      ...
      p.line(
        centerX + 10 * Math.cos(gearAngle),
        centerY - 41 + 10 * Math.sin(gearAngle),
        x,
        y,
      );
    }
    ...
    p.fill(120, 140, 140);
    p.text(
      "Final Design: Maia Sava, Cristian Ghidireac",
      centerX,
      p.height - 25,
    );
  };

  const drawPantographMechanism = (
    p: p5,
    baseX: number,
    baseY: number,
    extension: number,
    ...
    const p2 = {
      x: baseX + segmentLength * Math.cos(-angle),
      y: baseY - 15 - segmentLength * Math.sin(-angle),
    };
