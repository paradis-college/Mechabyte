    ...
    p5Instance = new p5((p: p5) => {
      ...
      function drawHangingMechanism(p: p5) {
        ...
        if (hookExtension > 10) {
          ...
          p.textSize(10);
          p.textAlign(p.LEFT);
          p.text(`${(hookExtension / 2).toFixed(0)}cm`, robotX + 15, hookY + 20);
        }
        ...
        if (hookExtension > 0) {
          ...
          p.textSize(11);
          p.textAlign(p.CENTER);
          p.text(`Extension: ${extPercent}%`, robotX, baseY - hookExtension - 20);
        }
      }
      ...
      function drawProgressBar(p: p5, progress: number) {
        ...
        p.textSize(11);
        p.textAlign(p.CENTER);
        p.text(`Phase ${phase + 1}/4 - ${(progress * 100).toFixed(0)}%`, 400, 592);
