...
const sketch = (p: p5) => {
  ...
  p.setup = () => {
    ...
    if (props.season === "into-the-deep") {
      samples = [
        { x: -80, y: 40, held: false, scored: false },
        { x: -60, y: 60, held: false, scored: false },
      ];
    } else {
  ...
  p.draw = () => {
    ...
    // Draw held sample
    if (sampleHeld)
      drawSample(p, scale, robotX, robotY - 20 - sliderHeight * 0.8);
  ...
  const updateAnimation = (phase: number) => {
    ...
    if (phase > cycleDuration) {
      ...
      samplesScored = 0;
      samples.forEach((s) => {
        s.held = s.scored = false;
      });
      controllerInputs.driver1 = {
        leftStick: { x: 0, y: 0 },
        rightStick: { x: 0, y: 0 },
        buttons: {},
      };
      controllerInputs.driver2 = {
        leftStick: { x: 0, y: 0 },
        rightStick: { x: 0, y: 0 },
        buttons: {},
      };
    ...
    if (props.season === "into-the-deep") {
      ...
      // TELEOP (3-16s): collect samples and score in high basket
        ...
        if (phase > 6 && !sampleHeld) {
          const nearSample = samples.find(
            (s) =>
              !s.held &&
              !s.scored &&
              Math.abs(s.x - robotX) < 25 &&
              Math.abs(s.y - robotY) < 25,
          );
          if (nearSample) {
  ...
  const drawSample = (p: p5, scale: number, x: number, y: number) => {
    ...
    const color =
      props.season === "into-the-deep"
        ? COLORS.SAMPLE_YELLOW
        : COLORS.PIXEL_PURPLE;
    ...
    if (props.season === "into-the-deep") {
      ...
      for (let i = 0; i < 6; i++) {
        ...
        p.vertex(
          sx + Math.cos(angle) * 10 * scale,
          sy + Math.sin(angle) * 10 * scale,
        );
      }
      p.endShape(p.CLOSE);
    } else {
  ...
  const drawControllers = (p: p5, scale: number) => {
    [0.25, 0.75].forEach((pos, i) => {
      ...
      drawController(
        p,
        scale * 1.1,
        i === 0 ? controllerInputs.driver1 : controllerInputs.driver2,
        i + 1,
      );
      p.pop();
    });
  ...
  const drawController = (
    p: p5,
    scale: number,
    inputs: typeof controllerInputs.driver1,
    driverNum: number,
  ) => {
    ...
    // Grips
    [-40, 40].forEach((x) =>
      p.ellipse(x * scale, 10 * scale, 25 * scale, 35 * scale),
    );
    ...
    [-30, 30].forEach((x, i) => {
      ...
      p.fill(
        active ? activeColor[0] : 70,
        active ? activeColor[1] : 75,
        active ? activeColor[2] : 80,
      );
      p.stroke(
        active
          ? p.color(...(activeColor as [number, number, number]))
          : p.color(100, 100, 100),
      );
      p.strokeWeight(active ? 3 * scale : 1 * scale);
      p.circle(
        x * scale + stick.x * 10 * scale,
        stick.y * 10 * scale,
        18 * scale,
      );
    });
    ...
    p.fill(
      dpadActive ? activeColor[0] : 60,
      dpadActive ? activeColor[1] : 65,
      dpadActive ? activeColor[2] : 70,
    );
    p.rect(-38 * scale, -23 * scale, 16 * scale, 6 * scale);
    p.rect(-33 * scale, -28 * scale, 6 * scale, 16 * scale);

    // Bumpers
    [inputs.buttons["left_bumper"], inputs.buttons["right_bumper"]].forEach(
      (active, i) => {
        p.fill(
          active ? activeColor[0] : 60,
          active ? activeColor[1] : 65,
          active ? activeColor[2] : 70,
        );
        p.rect(
          (i === 0 ? -50 : 20) * scale,
          -32 * scale,
...
<template>
  <div class="controller-robot-coordination">
    <div class="animation-header">
      <h4>
        {{ season === "into-the-deep" ? "Into the Deep" : "CenterStage" }} -
        Controller-Robot Coordination
      </h4>
      <p class="description">
        Simulation following game structure: Autonomous → Teleop → Endgame
      </p>
    </div>
