...
const sketch = (p: p5) => {
  ...
  p.draw = () => {
    ...
    // Progress bar
    drawProgressBar(p, phase, transitionTimer, armingTime, tensioningTime, flightTime);
  };
  ...
  const drawLauncher = (p: p5, phase: number, tension: number, dX: number, dY: number, rotation: number) => {
    ...
    for (const channel of channelPositions) {
      ...
      p.strokeWeight(2);
      p.line(centerX + channel.x - 12, centerY - 115, centerX + channel.x - 12, centerY + 115);
      p.line(centerX + channel.x + 12, centerY - 115, centerX + channel.x + 12, centerY + 115);
    ...
    if (phase < 2) {
      ...
      p.strokeWeight(3);
      for (let offset of [-3, 0, 3]) {
        p.line(centerX + offset, centerY - 110, centerX + offset, centerY + 100);
      }
    }
    ...
    if (phase === 0) {
      ...
      if (dY < 200) {
        p.textSize(11);
        p.fill(100, 200, 255);
        p.text(`Height: ${Math.max(0, Math.floor(-dY / 5))}m`, panelX, panelY + 10);
    ...
    p.textSize(10);
    p.fill(120, 120, 140);
    p.text('Designed by: Cristian Ghidireac, David Grigore', centerX, p.height - 25);
  };

  const drawDrone = (p: p5, x: number, y: number, rotation: number, inFlight: boolean) => {
    p.push();
    p.translate(x, y);
    p.rotate(rotation);
  ...
  const drawProgressBar = (p: p5, phase: number, timer: number, arm: number, tension: number, flight: number) => {
    const barWidth = 350;
    const barHeight = 8;
