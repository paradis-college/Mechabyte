...
const sketch = (p: p5) => {
  ...
  const drawBrushIntake = (p: p5) => {
    ...
    p.fill(200);
    p.text(
      "Spinning brush sweeps pixels up a ramp into storage",
      p.width / 2,
      85,
    );
    ...
    p.fill(200, 100, 100);
    p.text(
      "Issues: Required expansion hub (no motors available)",
      p.width / 2,
      p.height - 40,
    );
  };
  ...
  const drawClawV2 = (p: p5) => {
    ...
    p.fill(200, 100, 100);
    p.text(
      "Issues: Required precise alignment, single servo operation",
      p.width / 2,
      p.height - 40,
    );
  };
  ...
  const drawClawV3 = (p: p5) => {
    ...
    p.fill(200);
    p.text(
      "Counter-rotating toothed wheels create scissor action",
      p.width / 2,
      85,
    );
    ...
    p.fill(0, 255, 0);
    p.text(
      "Success: Easier alignment, forgiving capture angle",
      p.width / 2,
      p.height - 40,
    );
  };
...
<template>
  <div class="animation-wrapper">
    ...
    <p class="animation-caption">
      CenterStage Intake Evolution: From dynamic brush to precision claw to
      angled "crab claw"
    </p>
  </div>
</template>
