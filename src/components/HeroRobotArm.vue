...
const derivatives = (
  ...
  const den1 =
    (m1 + m2 + m3) * L1 -
    m2 * L1 * Math.cos(delta12) * Math.cos(delta12) -
    m3 * L1 * Math.cos(delta13) * Math.cos(delta13);
  const den2 = (m2 + m3) * L2 - m3 * L2 * Math.cos(delta23) * Math.cos(delta23);
  const den3 = m3 * L3;

  // Simplified acceleration terms (Lagrangian mechanics)
  const num1 =
    -g * (2 * m1 + m2 + m3) * Math.sin(t1) -
    m2 * g * Math.sin(t1 - 2 * t2) -
    m3 * g * Math.sin(t1 - 2 * t3) -
    2 *
      Math.sin(delta12) *
      m2 *
      (o2 * o2 * L2 + o1 * o1 * L1 * Math.cos(delta12)) -
    2 *
      Math.sin(delta13) *
      m3 *
      (o3 * o3 * L3 + o1 * o1 * L1 * Math.cos(delta13));
  ...
  const num2 =
    2 *
      ...
      (o1 * o1 * L1 * (m2 + m3) +
        g * (m2 + m3) * Math.cos(t1) +
        o2 * o2 * L2 * m2 * Math.cos(delta12)) -
    m3 * g * Math.cos(t2) * Math.sin(delta23) -
    m3 * o3 * o3 * L3 * Math.sin(delta23);

  const num3 =
    2 * Math.sin(delta13) * (o1 * o1 * L1 * m3 + g * m3 * Math.cos(t1)) +
...
const rk4Step = (dt: number) => {
  ...
  theta3 +=
    (dt / 6) * (k1.dTheta3 + 2 * k2.dTheta3 + 2 * k3.dTheta3 + k4.dTheta3);
  omega1 +=
    (dt / 6) * (k1.dOmega1 + 2 * k2.dOmega1 + 2 * k3.dOmega1 + k4.dOmega1);
  omega2 +=
    (dt / 6) * (k1.dOmega2 + 2 * k2.dOmega2 + 2 * k3.dOmega2 + k4.dOmega2);
  omega3 +=
    (dt / 6) * (k1.dOmega3 + 2 * k2.dOmega3 + 2 * k3.dOmega3 + k4.dOmega3);
};
...
<template>
  ...
  >
    ...
    >
      ...
      <desc>
        A mechanical robot arm that follows your cursor and performs a subtle
        idle animation
      </desc>

      <!-- Base/shoulder -->
      <circle cx="100" cy="150" r="20" class="robot-base" />
      ...
      >
        ...
        <!-- First joint -->
        <circle cx="100" cy="90" r="8" class="robot-joint" />
        ...
        >
          ...
          <!-- Second joint -->
          <circle cx="100" cy="40" r="7" class="robot-joint" />
